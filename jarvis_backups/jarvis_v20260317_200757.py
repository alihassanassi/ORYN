...
try:
    from audio.sound_engine import play as _play_sound, start as _sound_start
except Exception:
...
class JARVIS(QMainWindow):
    ...
    def _build_ui(self):
        ...
        root_v.addWidget(self._mk_input_bar())
    ...
    def _mk_right(self) -> QWidget:
        ...
        v.addWidget(self._sec_hdr("◈  SELF-EVOLUTION"))

        self._evo_analyze_btn = QPushButton("🧬  ANALYSE & EVOLVE")
    ...
    def _mk_wave_row(self) -> QWidget:
        ...
        self._wave = WaveformVisualizer()
        row.addWidget(self._wave, 1)

        self._mic_btn = QPushButton("🎤")
        self._mic_btn.setFixedSize(38, 38)
        self._mic_btn.setCheckable(True)
        self._mic_btn.setCursor(QCursor(Qt.PointingHandCursor))
        self._mic_btn.setToolTip("Push-to-Talk  (click to start, click again to stop)")
        self._mic_btn.setStyleSheet(f"""
            QPushButton {{
                background:transparent; color:{P['t2']};
                border:1px solid {P['b1']}; border-radius:19px;
                font-size:16px; padding:0;
            }}
            QPushButton:hover {{ border-color:{P['arc_d']}; color:{P['arc']}; }}
            QPushButton:checked {{
                background:{P['arc_g']}; border-color:{P['arc']}; color:{P['arc']};
            }}
        """)
    ...
    def _on_persona_btn_click(self, key: str) -> None:
        ...
        try:
            from voice.profiles import get_profile_for_persona as _gp
            self._tts.set_profile(_gp(key).name)
    ...
    def _on_persona_switch(self, persona_key: str) -> None:
        ...
        if hasattr(self, '_orb'):
            ...
            self._orb.set_state(f"{meta['display'].upper()} PROTOCOL ACTIVE", "SYSTEM RECONFIGURED")
    ...
    def _ptt_start(self):
        ...
        self._telemetry.set_voice_status("🔴  Listening…", P["arc"])
        if hasattr(self, '_orb'):
    ...
    def _on_auto_toggle(self, on: bool):
        if on:
            ...
            self._add_msg(
                ...
                "I'll monitor your operations every 5 minutes and surface tasks for your approval. "
                "Nothing executes without your explicit confirmation."
            )
        else:
