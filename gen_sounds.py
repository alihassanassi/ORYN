...
def fade(s,i=.01,o=.05):
    ...
    return s
print("Generating JARVIS sounds...")
h=chirp(60,180,.8,.4); b=chirp(120,360,.8,.2); cr=noise(.8,.15)*np.exp(-t(.8)*3)
save("boot_start.wav", fade((h+b+cr)*np.linspace(0,1,int(SR*.8))**.3,.01,.08))
thud=sine(55,.3,.8)*np.exp(-t(.3)*8); swell=chirp(220,880,.7,.35)+sine(440,.7,.2)+sine(660,.7,.1)
swell*=np.linspace(0,1,int(SR*.7))**.4; full=np.zeros(int(SR*1.0))
full[:len(thud)]+=thud; full[int(SR*.1):int(SR*.1)+len(swell)]+=swell
save("boot_complete.wav", fade(full,.005,.15))
s=chirp(440,55,1.2,.5)+chirp(220,28,1.2,.3); s*=np.exp(-t(1.2)*1.5)
save("shutdown.wav", fade(s,.01,.2))
save("kill_switch.wav", fade(np.concatenate([noise(.05,.9),noise(.03,.5),sine(110,.4,.6)*np.exp(-t(.4)*6),np.zeros(int(SR*.1))]),.001,.05))
save("kill_switch_arm.wav", fade(np.concatenate([noise(.02,.7)*np.exp(-t(.02)*50),sine(330,.08,.3)*np.exp(-t(.08)*15)]),.001,.02))
save("click.wav", fade((noise(.008,.6)+sine(800,.015,.3))*np.exp(-t(.023)*80),.0005,.008))
save("transmit.wav", fade((chirp(880,1200,.08,.4)+sine(1100,.08,.2))*np.exp(-t(.08)*8),.002,.015))
save("tab_switch.wav", fade(chirp(200,600,.12,.35)*np.exp(-t(.12)*8)+noise(.12,.06)*np.exp(-t(.12)*12),.002,.02))
...
save("settings_open.wav", fade(chirp(300,500,.18,.3)+noise(.18,.06)*np.exp(-t(.18)*8),.005,.03))
...
save("node_click.wav", fade(sine(660,.1,.35)*np.exp(-t(.1)*18)+sine(990,.1,.15)*np.exp(-t(.1)*25),.001,.02))
...
s=np.zeros(int(SR*.5))
for st,f,d in [(.0,800,.06),(.08,1200,.05),(.15,1000,.07),(.25,1400,.04),(.31,900,.08)]:
    i=int(SR*st); tt2=t(d); wb=1+.03*np.sin(2*np.pi*15*tt2)
...
save("finding_low.wav", fade(sine(880,.3,.3)*np.exp(-t(.3)*6)+sine(1320,.3,.15)*np.exp(-t(.3)*8),.002,.04))
save("finding_confirmed.wav", fade((chirp(400,1200,.25,.5)+chirp(800,2400,.25,.2)+noise(.25,.1)*np.exp(-t(.25)*10))*np.exp(-t(.25)*4),.001,.04))
s=np.zeros(int(SR*.8))
for r in range(4):
    st=int(SR*r*.2); a1=sine(660,.08,.5)*np.exp(-t(.08)*10); a2=sine(550,.08,.5)*np.exp(-t(.08)*10)
    s[st:st+len(a1)]+=a1; s[st+int(SR*.1):st+int(SR*.1)+len(a2)]+=a2
save("cve_match.wav", fade(s,.005,.06))
s=np.zeros(int(SR*.6))
for r in range(3):
    st=int(SR*r*.18); k=(sine(440,.12,.6)+sine(466,.12,.4))*np.exp(-t(.12)*5); s[st:st+len(k)]+=k
...
save("scan_start.wav", fade((chirp(100,2000,.4,.5)+chirp(150,3000,.4,.25)+noise(.4,.1)*np.linspace(0,1,int(SR*.4)))*np.linspace(0,1,int(SR*.4)),.01,.03))
s=chirp(2000,200,.35,.5)+chirp(3000,300,.35,.25); thump=sine(80,.2,.7)*np.exp(-t(.2)*8)
...
for st,f,d in [(.0,1000,.05),(.07,1400,.04),(.13,1200,.05)]:
    i=int(SR*st); tone=sine(f,d,.35)*np.exp(-t(d)*15); s[i:i+len(tone)]+=tone
save("tool_complete.wav", fade(s,.001,.03))
s=chirp(55,220,.6,.5)+sine(110,.6,.3)*np.linspace(0,1,int(SR*.6)); s*=np.linspace(0,1,int(SR*.6))**.5
...
save("report_draft.wav", fade(chirp(330,550,.2,.3)+noise(.2,.08)*np.exp(-t(.2)*5),.005,.03))
...
save("memory_pin.wav", fade(noise(.012,.5)*np.exp(-t(.012)*80)+sine(600,.04,.3)*np.exp(-t(.04)*20),.0005,.01))
...
s=np.zeros(int(SR*.3))
for st,f,d in [(.0,1000,.04),(.06,1400,.03),(.11,1800,.03),(.16,1400,.05)]:
    i=int(SR*st); tt2=t(d); wb=1+.04*np.sin(2*np.pi*18*tt2)
    ph=2*np.pi*np.cumsum(f*wb)/SR; tone=.4*np.sin(ph[:len(tt2)])*np.exp(-tt2*12); s[i:i+len(tone)]+=tone
